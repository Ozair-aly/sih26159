...
def analyze_pcap(raw):
    ...
    return {"product":"Mail Vault","problem_statement":"SIH26159","generated_at":datetime.now(timezone.utc).isoformat(),
...
@app.get("/api/health")
def health(): return {"status":"ok","service":"Mail Vault"}
...
async def html_report(payload:dict):
    ...
    html=f"""<!doctype html><html><head><meta charset=utf-8><title>Mail Vault Report</title>
    ...
    <body><h1>Mail Vault</h1><h2>Cryptographic Security Posture Assessment</h2>
    ...
    <p>Generated by Mail Vault — SIH 26159 prototype.</p></body></html>"""
...
async def pdf_report(payload:dict):
    try:
        ...
        story=[Paragraph("Mail Vault",st["Title"]),Paragraph("Cryptographic Security Posture Assessment — SIH 26159",st["Heading2"]),
        ...
        doc.build(story); return Response(b.getvalue(),media_type="application/pdf",headers={"Content-Disposition":"attachment; filename=mail-vault-report.pdf"})
